...
Properties: set of property functions `C = {C1, ... Cn}`  
Property: function `Ci: E → Di`  
...
Should the program check that `Li` never overlap on start?
Should it check instead that for each `e in E` used, `∃! i | C(e) in Li`?

Should the program check that `union Li` covers `D`?
...
### Allowing changes to the set of properties `C`

What if `C` doesn't agree between partitions?  
→ `C` is a property of the whole repo, specified once by the user. Partitions use a subset.

What if a user changes `Ci(e)` for some `Ci`, `e`?  
→ User must notify that some partitions may now contain incorrect elements  
→ It may make sense to adjust `L` to minimise relocation work
→ Search and filtering may not work until all elements have been moved

What if a new property `Ci` is added?  
→ No action needed since existing partitions ignore unused properties

What if a property `Ci` is removed?  
→ If no partition uses it, no problem  
→ If a partition `Pi` depends on it, its `Li` must be updated; it may be necessary to replace usage
    with a new Ci or to merge with another partition
→ Search and filtering may not work until all elements have been moved
...
Note that the specification of `L` depends on `P` and `C`. All three may be specified directly by
the user, or `P` discovered and the rest inferred then specified by the user, or `P` and `L` may be
...
required that this set, `P`, be known at start-up, but it is useful to have a way to discover `P`.

In the case that all data files are in a single directory or directory tree whose location is
given it is very easy to discover `P`, though some assurance that data files correspond to the same
repository may be useful.

### Specifying `C`

Classification functions must be provided as Rust functions by the library user. There is no harm
in providing unused functions. Therefore there should not be any discovering to do, only specifying
by the user.

Data files could (and probably should) list identifiers for required classification functions. This
could be used to load function code on demand and to fault-check missing properties.

### Discovering `L`

This can be divided into two subproblems: discovering `Li` for the current partition `i`, and
discovering `Lj` for other partitions (or the whole of `L`).
...
that information about `L` must be distributed.

Each partition `i` may store information about its own partitioning (`Li`), but information stored
about other partitions can never be complete and may be outdated. Partition `i` can be used without
knowing about other partitions, so long as the classifications `L` have no overlaps among "live"
partitions and each partition can mark itself "dead" or as matching a reduced set of
classifications (i.e. partially or completely superseded by other partition(s)).

To find existing or insert new elements with classifications not covered by known (classified)
partitions, the classification of (some) other partitions needs to be discovered.
...
Another option: store a description of the known subset of `P` in each partition's metadata.  
→ Not very complex  
← May result in large headers with lots of redundency  
← Allows multiple different specifications of `Li` for some `i`, thus either each `Li` must be
fixed or the active description updated when that partition is loaded  
← Finding elements of unknown location can be fast when most of `L` is known at the time metadata
is written, but still expected `O(n)` where `L` is mostly unknown during writing
...
→ Accelerates discovery of `P`
← Does not help discover `L`
...
← Doesn't support fully dynamic, distributed partitioning unless user can discover `L` from file
names; a compromise using a centralised description of `L` may work reasonably in practice.
...
Extending `C` with new or newly known properties is easy. Removing properties from `C` is easy.
Operation without all properties may mean that membership `C(e) in Li` is not computable, and
...
This implies that the `Li` for each partition `i` should not change; in other words, it should be
