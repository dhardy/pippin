...
pub fn repo_from_path(path: &Path) -> Result<RepoFileIO> {
    ...
    for entry in WalkDir::new(dir) {
        ...
        let (pn, mut part_paths) = match prefixes.entry(prefix) {
            ...
            Entry::Vacant(e) => {
                let fname = try!(entry.file_name().to_str().ok_or(
                    PathError::new("not valid UTF-8", path.to_path_buf())));
                let pn = try!(find_part_num(fname, &path));
                e.insert(pn);
